  ...
  return (
    <div className="p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {results.map(item => (
          <Link to={`/truyen/${item.slug}`} key={item._id} className="bg-white rounded shadow p-2 hover:shadow-lg transition text-black">
            <img
              src={item.thumb_url ? `https://img.otruyenapi.com/uploads/comics/${item.thumb_url}` : "https://via.placeholder.com/150x220?text=No+Image"}
              alt={item.name}
      ...
      {!loading && results.length === 0 && <div className="text-gray-400 mt-4">Không tìm thấy truyện phù hợp.</div>}
    </div>
  );
};
